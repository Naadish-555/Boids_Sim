...
void Game::sMovement()
{
	...
	for (auto& e : m_entities.getEntities())
	{
		/*if ((player()->get<CShape>().circle.getGlobalBounds().top < 0 && player()->get<CTransform>().velocity.y < 0) || (player()->get<CShape>().circle.getGlobalBounds().top + player()->get<CShape>().circle.getGlobalBounds().height >= m_windowConfig.WH && player()->get<CTransform>().velocity.y > 0))
		{
			player()->get<CTransform>().velocity.y = 0;
		}

		if ((player()->get<CShape>().circle.getGlobalBounds().left < 0 && player()->get<CTransform>().velocity.x < 0) || (player()->get<CShape>().circle.getGlobalBounds().left + player()->get<CShape>().circle.getGlobalBounds().width >= m_windowConfig.WW && player()->get<CTransform>().velocity.x > 0))
		{
...
void Game::sCollision()
{
	...
	for (auto &e : m_entities.getEntities())
	{
		/*if (e->tag() == "Enemy" || e->tag() == "SmallEnemy")
		{
			...
			{
				float diff = e->cTransform->pos.distsq(b->cTransform->pos);
			...
			{
				float diff = e->cTransform->pos.distsq(m_player->cTransform->pos);
	...
	//		for (auto& b : m_entities.getEntities("Bullet"))
	//		{
	//			float diff = e->get<CTransform>().pos.distsq(b->get<CTransform>().pos);
	...
	//		{
	//			float diff = e->get<CTransform>().pos.distsq(player()->get<CTransform>().pos);
	...
	//	}
		...
		/*if (e->tag() == "Enemy")
		{
			if ((e->get<CShape>().circle.getGlobalBounds().top < 0 && e->get<CTransform>().velocity.y < 0) || (e->get<CShape>().circle.getGlobalBounds().top + e->get<CShape>().circle.getGlobalBounds().height >= m_windowConfig.WH && e->get<CTransform>().velocity.y > 0))
			{
				e->get<CTransform>().velocity.y = -(e->get<CTransform>().velocity.y);
			}

			if ((e->get<CShape>().circle.getGlobalBounds().left < 0 && e->get<CTransform>().velocity.x < 0) || (e->get<CShape>().circle.getGlobalBounds().left + e->get<CShape>().circle.getGlobalBounds().width >= m_windowConfig.WW && e->get<CTransform>().velocity.x > 0))
			{
	...
	for (int i=0;i<boids.size();i++)
	{
		...
		for (auto& index : nearbyIndices)
		{
			...
			if (dist < (m_visionDistance * m_visionDistance))
			{
				...
				//seperation
				if (dist < (m_avoidDistance * m_avoidDistance))
				{
					Vec2 diff = bPos - oPos;
					seperation += diff / (dist + 0.1f); //closer they are stronger the push , adding 0.1f to ensure denominator is never zero
				}
			}
		}
		...
		//apply to velocity
		bVel += totalForce * m_agility; //small factor to smooth movement
...
void Game::spawnSmallEnemies(std::shared_ptr<Entity> e)
{
	...
	//	//float disperseAngle = ((static_cast<float>(360) / e->get<CShape>().circle.getPointCount()) * i) * 3.14159 / 180;
...
void Game::spawnBoid()
{
	...
	float sx = (static_cast<float>(rand()) / RAND_MAX) * (m_enemyConfig.SMAX - m_enemyConfig.SMIN) + (m_enemyConfig.SMIN); //random float number generation between max and min value
	float sy = (static_cast<float>(rand()) / RAND_MAX) * (m_enemyConfig.SMAX - m_enemyConfig.SMIN) + (m_enemyConfig.SMIN);
